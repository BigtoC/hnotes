...
class DrawerWidget extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    return new Drawer(
      child: Column(
        ...
        children: <Widget>[
          header(context),
          navigatorTab(context, Icons.favorite, Colors.pinkAccent, "Day Counts", tapDayCount),
          navigatorTab(context, Icons.cloud, Colors.blueAccent, "Blockchain Info", tapBlockchain),
          navigatorTab(context, Icons.settings, Colors.purpleAccent, "Settings", tapSettings),
          navigatorTab(context, Icons.book, Colors.green, "Usage Guide", tapUsageGuide),
          sizeBox(context, 0.1),
          divider(context, 0.5),
          version(),
        ],
  ...
  Widget _dayCountNumber(BuildContext context, String number) {
    return GestureDetector(
        ...
        child: Center(
          child: Text(
            number,
            style: TextStyle(fontSize: 60.0, fontWeight: FontWeight.w500, color: Colors.white),
          ),
        )
    );
  }
  ...
  Widget navigatorTab(
    ...
    return ListTile(
      ...
      title: Text(
        title,
        style: TextStyle(fontSize: subtitleFontSize, fontWeight: itemFontWeight),
      ),
      onTap: () {
        onTapAction(context);
      },
